...
import { structureIdeaAI, validateIdea } from "../lib/pitchAi";
...
router.post("/ideas", async (req: Request, res: Response): Promise<void> => {
  ...
  const [idea] = await db
    .insert(ideasTable)
    .values({ userId, title: parsed.data.title, rawText: parsed.data.rawText, status: "draft" })
    .returning();
...
router.get("/ideas/:id", async (req: Request, res: Response): Promise<void> => {
  ...
  const userId = req.user!.id;
  const [idea] = await db.select().from(ideasTable).where(and(eq(ideasTable.id, params.data.id), eq(ideasTable.userId, userId)));
  if (!idea) { res.status(404).json({ error: "Idea not found" }); return; }
  res.json(GetIdeaResponse.parse(idea));
});

router.patch("/ideas/:id", async (req: Request, res: Response): Promise<void> => {
...
router.delete("/ideas/:id", async (req: Request, res: Response): Promise<void> => {
...
router.post("/ideas/:id/structure", async (req: Request, res: Response): Promise<void> => {
  const params = StructureIdeaParams.safeParse(req.params);
  if (!params.success) { res.status(400).json({ error: params.error.message }); return; }
  const userId = req.user!.id;
  const [existing] = await db.select().from(ideasTable).where(and(eq(ideasTable.id, params.data.id), eq(ideasTable.userId, userId)));
  if (!existing) { res.status(404).json({ error: "Idea not found" }); return; }

  // AI-powered structuring — reads raw text and produces investor-grade sections
  const structured = await structureIdeaAI(existing.title, existing.rawText);

  const [updated] = await db
    .update(ideasTable)
    .set({ ...structured, status: "structured", updatedAt: new Date() })
    .where(eq(ideasTable.id, existing.id))
    .returning();
  res.json(StructureIdeaResponse.parse(updated));
});

router.post("/ideas/:id/validate", async (req: Request, res: Response): Promise<void> => {
  ...
  await db
    .update(ideasTable)
    .set({ validationScore: result.score, validationStrengths: result.strengths, validationWeaknesses: result.weaknesses, validationSuggestions: result.suggestions, updatedAt: new Date() })
