// Heuristic, deterministic "AI" used for the first build so the platform
// works end-to-end with no external API key. The shape of the output mirrors
// the OpenAPI schemas so the responses validate cleanly. This module is
...
function pickAudience(text: string): string {
  ...
  const hits: string[] = [];
  if (/student|college|school|teen|young/.test(lower))
    hits.push("students and young founders");
  if (/teacher|educator|professor/.test(lower)) hits.push("educators");
  if (/business|enterprise|company|saas|b2b/.test(lower))
    hits.push("small and mid-sized businesses");
  if (/parent|family|kid|child/.test(lower)) hits.push("parents and families");
  if (/developer|engineer|programmer/.test(lower))
    hits.push("software builders");
  if (hits.length === 0) hits.push("early adopters in the target domain");
  return hits.join(", ");
}

export function structureIdeaText(
  title: string,
  rawText: string,
): StructuredIdea {
...
export function validateIdea(idea: Idea): IdeaValidationResult {
  ...
  const filled = fields.filter(
    (f) => typeof idea[f] === "string" && (idea[f] as string).length > 30,
  );
  ...
  if (completeness >= 0.8) {
    ...
    weaknesses.push(
      "Several core sections are missing or thin (problem, solution, market, business model, edge, audience).",
    );
    suggestions.push(
      "Run the AI structuring step, then sharpen each section with one concrete example or number.",
    );
  }

  if (richness >= 0.6) {
    strengths.push(
      "The raw description gives the AI enough context to reason about details.",
    );
  } else {
    weaknesses.push(
      "Raw idea description is short — investors will need more context to grade you.",
    );
    suggestions.push(
      "Expand the raw idea: who is the user, what is the moment of pain, and why now.",
    );
  }
  ...
  if (idea.competitiveEdge && idea.competitiveEdge.length > 60) {
    ...
    suggestions.push(
      "Name your unfair advantage in one sentence: a workflow, dataset, distribution, or insight only you have.",
    );
  }

  if (idea.businessModel && idea.businessModel.toLowerCase().includes("free")) {
    suggestions.push(
      "Quantify your freemium funnel: what % of free users convert, and to what ARPU.",
    );
  }

  if (strengths.length === 0)
    strengths.push("You showed up and put a real idea on the page — that's step one.");
...
export function generateDeckSlides(idea: Idea): {
  ...
  const slides: DeckSlide[] = [
    ...
    },
    {
      ...
      body:
        idea.problem ??
        "Users today struggle with a workflow that nobody owns end to end.",
      layout: "problem",
      bullets: [
    ...
    },
    {
      ...
      body:
        idea.solution ??
        "A single product that owns the full journey, not just one step.",
      layout: "solution",
      bullets: [
    ...
    },
    {
      ...
      body:
        idea.businessModel ??
        "Freemium core, paid upgrades for advanced workflows, organization plans for distribution.",
      layout: "business",
    ...
    },
    {
      ...
      body:
        ...
        "Unlike point tools, we own the full loop — and the data that compounds inside it.",
      layout: "edge",
...
export function pickInvestorQuestion(slug: string, turn: number): string {
  const bank = QUESTION_BANK[slug] ?? QUESTION_BANK["curious-angel"]!;
...
export function scorePitchTurn(content: string): {
  ...
  // Clarity: prefer 12-22 word sentences, penalize fillers
  ...
  if (clarity > 78 && confidence > 78) {
    feedback =
      "Sharp delivery — your point landed in one breath. Keep that pace and tighten one specific number into the next answer.";
    ...
    feedback =
      "Drop the hedging language. Replace 'I think' with 'we know' and back it with one concrete data point.";
  } else if (avgSentenceLength > 25) {
    feedback =
      "Sentences are running long. Cut each one in half — investors lose your thesis when you stack clauses.";
  } else if (wordCount < 25) {
    feedback =
      "Too short. Investors need the why, the what, and the proof in one breath. Add a concrete example.";
  } else {
    feedback =
      "Solid turn. Tighten the close — finish on the strongest sentence, not the longest one.";
  }
...
export function summarizeSession(
  ...
  if (userTurns.length === 0) {
    return {
      ...
      summary:
        "No user turns recorded — start a new session and pitch at least once to get a real read.",
      mistakes: [],
    };
  }
  ...
  const totalFillers = userTurns.reduce(
    (a, t) => a + (t.fillerWords ?? 0),
    0,
  );
  const investorReadiness = Math.max(
    ...
    Math.min(
      99,
      Math.round(confidenceScore * 0.45 + clarityScore * 0.45 - totalFillers * 1.5),
    ),
  );
  ...
  if (confidenceScore < 65) {
    mistakes.push({
      ...
      description:
        "Your phrasing leaned on hedging like 'I think' and 'maybe'. That tells the room you don't fully believe the thesis.",
      suggestion:
        "Rewrite each answer to lead with a definitive statement. Open with the strongest sentence, not the safest.",
      severity: "high",
    });
  }
  ...
  if (clarityScore < 65) {
    mistakes.push({
      ...
      description:
        "Several answers ran long or buried the point. The investor lost the thesis halfway through.",
      suggestion:
        "Use the formula: claim, proof, why-it-matters. One sentence each. Stop talking after the third.",
      severity: "medium",
    });
  }

  const shortTurns = userTurns.filter(
    (t) => t.content.trim().split(/\s+/).length < 25,
  ).length;
  if (shortTurns >= Math.max(2, userTurns.length / 2)) {
    mistakes.push({
      title: "Answers under-developed",
      description:
        "You answered in fragments. Investors need the what, the why, and the proof — every time.",
      suggestion:
        "For each question, force yourself to add one concrete example, one number, and one sentence about what it means.",
      severity: "medium",
    });
  }
